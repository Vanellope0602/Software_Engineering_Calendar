...
class LoginForm(FlaskForm):
	username = StringField('昵称', validators=[InputRequired(message='用户名不能为空噢'), Length(min=4, max=15)])
	password = PasswordField('密码', validators=[InputRequired(message='密码不能为空噢'), Length(min=8, max=80)])
	remember = BooleanField('remember me')

class RegisterForm(FlaskForm):
	email = StringField('邮箱', validators=[InputRequired(message='邮箱不能为空噢'),Email(message='这个不是正确格式的邮箱诶'), Length(max=50)])
	username = StringField('昵称', validators=[InputRequired(message='用户名不能为空噢'), Length(min=4, max=15)])
...
class EventForm(FlaskForm):
	...
	type = SelectField('请选择类别', validators=[InputRequired()],
					   ...
					   id='typearea')
	start = DateTimeField('起始时间（格式：yyyy/mm/dd hh:mm）', default=datetime.now() ,validators=[InputRequired(message='起始日期不能为空噢')], format='%Y/%m/%d %H:%M',id='datetime-start')
	end = DateTimeField('结束时间（格式：yyyy/mm/dd hh:mm）', default=datetime.now() ,validators=[InputRequired(message='结束日期也不能为空噢')] ,format='%Y/%m/%d %H:%M',id='datetime-end')
	descirbe = StringField('详细备注') # 可以留空
	submit = SubmitField('提交日程')
...
def login():
	...
	if form.validate_on_submit():
		...
		if user:
			# 用户存在，比较密码，左侧为数据库中用户密码的哈希，右侧为登录表单时用户输入的密码
...
def edit(event_id=None):
	...
	if form.validate_on_submit():
		if form.start.data <= form.end.data:
			...
			new_event = Event(title=form.event_title.data,
							  url='http://127.0.0.1:5000/edit', # Todo: reverse??
							  type=form.type.data,
							  start_time=form.start.data,
							  end_time=form.end.data,
							  descirbe=form.descirbe.data,
	...
	upcoming = []
	cnt = 0
	for tmpe in all_event_list:	# all event已经是按照起始时间排好顺序的了
		if tmpe.start_time >= datetime.now():
			if cnt < 3:
				upcoming.append(tmpe)	# 事件只传送最近三个
			cnt = cnt + 1
	...
	return render_template('list_and_edit.html', form=form, search_form=search_form,
						   ...
						   search=result,res_len=len(result), next=upcoming,next_num=cnt)
...
def descirbe(event_id):
	...
	# 先初始化预设一个值
	edit_form = EventForm(event_title=one_event.title,
						  type=one_event.type,
						  start=one_event.start_time,
						  end=one_event.end_time,
						  descirbe=one_event.descirbe)
	print("初始化成功！")
	if edit_form.validate_on_submit():
		print("进入了修改日程的validate函数")
		if edit_form.start.data <= edit_form.end.data:
			one_event.title = edit_form.event_title.data
			one_event.type = edit_form.type.data
			one_event.start_time = edit_form.start.data
			one_event.end_time = edit_form.end.data
			one_event.descirbe = edit_form.descirbe.data
			...
			flash("结束时间不得早于起始时间")
			print("修改日程时结束时间不得早于起始时间")

	return render_template('one_event.html', event=one_event, form=edit_form)
